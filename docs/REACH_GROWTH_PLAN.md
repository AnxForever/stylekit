...
| # | Action | Owner | Status |
|---|--------|-------|--------|
| 0.1 | Expand npm keywords on all 3 packages (core 11→32, mcp 10→21, cli 7→18); fix stale "146"→"148" in descriptions | me (done) | done edited and published |
| 0.2 | Fix stale "146"→"148" across README.md, README.zh-CN.md, public/llms.txt, feishu README | me (done) | done |
| 0.3 | Bump versions so keyword changes can publish (core→1.0.0-beta.4, mcp→0.2.1, cli→0.1.3) | me (done) | done |
| 0.4 | Publish all 3 packages (core beta.4 --tag beta, mcp 0.2.1, cli 0.1.3) — keywords now live on npm | me (done via logged-in npm) | done published + verified |
| 0.4b | Re-publish with `--provenance` — folded into Wave 1 CI | Wave 1 | MCP completed; CLI/core workflows ready for their next version tags |
| 0.5 | Fill 20 GitHub repo topics | me (done via gh) | done verified in repo |
...
| 1.3 | CI job to publish to npm (--provenance) + MCP Registry (OIDC) on `mcp-v*` tag; same for cli on `cli-v*` | me | mcp-publish.yml completed; cli-publish.yml hardened |
| 1.4 | GitHub topics `mcp` + `model-context-protocol` → Glama auto-indexes | me | done (Wave 0.5) |
| 1.5 | README structured for directory auto-parse (`## Tools` table, npx snippet, fixed stale 0.2.0/0.1.1 refs) | me | done |
| 1.6 | **Publish to Official Registry** — publish the versioned MCP server record after npm propagation | me | done on 2026-09-06 as `io.github.AnxForever/stylekit-mcp` v0.2.5 |
...
| 2.8 | Baidu push-API script (`pnpm submit:baidu`, new/updated URLs only, logs remain) | me | done built + tested (submit-baidu.mjs) |
...
| # | Action | Owner | Status |
|---|--------|-------|--------|
| 2.9 | Audit prod nginx logs for all major crawlers | me | done **VERDICT: healthy, no bleed** — all reach fine; Baiduspider already crawling clean (0 404s). See WAVE2_SEARCH_ENGINES.md 2C |
| 2.10 | CDN edge/mirror if crawlers starved | — | done not needed (2.9 cleared it) |

### 2D. Baiduspider rendering audit
| # | Action | Owner | Status |
|---|--------|-------|--------|
| 2.11 | Confirm style / colors / GEO pages are server-rendered (content + JSON-LD in raw HTML) | me | done verified: style 269+ body hits & 2 JSON-LD; colors 162 & 2; GEO 275 & 2 — all in raw HTML |
...
| 2.15 | Answer-first opening paragraphs | me | done audited — already in good shape: style pages open h1→definitional description; dark-mode GEO intro opens "Dark mode design is more than inverting colors… (4.5:1)". FAQPage schema already on style pages. No mass rewrite warranted. |
| 2.16 | Entity clarity: Organization + sameAs schema | me | done entity graph already present (WebApplication/Organization/WebSite w/ sameAs). Added the 3 npm package URLs to Organization.sameAs to tie the entity to its npm distribution. Canonical boilerplate defined in WAVE1_MCP_SUBMISSIONS.md for cross-channel use. |
...
| # | Action | Owner | Status |
|---|--------|-------|--------|
| 3.1 | Chinese dev communities + AlternativeTo/SaaSHub/awesome PRs + Show HN | **you** (post) | done all copy drafted in WAVE3_COMMUNITY_LAUNCH.md (掘金 deep-dive, 知乎 answer, V2EX 推广, GitHub weeklies, listings, awesome PRs, Show HN — distinct angle each) |

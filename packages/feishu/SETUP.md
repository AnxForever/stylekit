...
机器人会从 146 个设计风格里挑出最合适的 3 个候选，流式输出推理过程，点卡片按钮选定风格后，把 23 件产物的工程包（AI 提示词、Cursor/Claude 规则文件、shadcn 主题、Tailwind preset、Figma tokens、组件代码等）打包 ZIP 上传、写进多维表格并生成一份飞书文档。

写完代码贴回群里，机器人按该风格的规则引擎体检代码，逐条指出违规类名并给出修复建议，结果回写表格。

**多维表格零配置**：没有配置 `FEISHU_BASE_TOKEN` 时，第一次交付会自动建一张「StyleKit 风格台账」，后续交付和体检结果都写进去。

## 常见问题

- **写入多维表格报 91403**：在 Base 右上角 ⋯ → 更多 → 添加文档应用，把应用加进来并授予「可编辑」。（零配置自动建的表由应用自己创建，通常没有这个问题。）
- **群里 @了没反应**：确认权限已发布新版本；确认机器人已在群里；长连接模式下等 1 分钟生效。
- **机器人突然没反应**：大概率是长连接断线（本机网络抖动）。重启 `pnpm --filter stylekit-feishu dev` 即可 —— 单实例锁会防止双开。

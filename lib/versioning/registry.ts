...
import type { StyleVersion, StyleVersionHistory, StyleVersionRegistry } from "./types";
...
export function getCurrentVersion(slug: string): string {
  return versionData[slug]?.current ?? "0.0.0";
}
...
export function getChangelog(slug: string): StyleVersion[] {
  return versionData[slug]?.versions ?? [];
}

/**
 * Get the complete version registry for all styles.
 */
export function getAllVersions(): StyleVersionRegistry {
  return versionData;
}

...
// 从完整风格数据中提取的展示所需字段
export interface StyleDisplayData {
  slug: string;
  name: string;
  colors: {
    primary: string;
...
// Color Compare Panel
// ============================
function ColorComparePanel({ colors }: { colors: StyleDisplayData["colors"] }) {
  const colorItems = [
    { label: "Primary", value: colors.primary },
    { label: "Secondary", value: colors.secondary },
    ...colors.accent.map((c, i) => ({ label: `Accent ${i + 1}`, value: c })),
  ];
...
// Rules Compare Panel
// ============================
function RulesComparePanel({ doList, dontList }: { doList: string[]; dontList: string[] }) {
  return (
...
// ============================
function PhilosophyComparePanel({ philosophy }: { philosophy: string }) {
  return (
...
function CompareInner({
  ...
  stylesData: Record<string, StyleDisplayData>;
}) {
  const searchParams = useSearchParams();
  const router = useRouter();

  // Read from URL, fallback to defaults
  const leftSlug = searchParams.get("left") || stylesMeta[0]?.slug || "";
  const rightSlug = searchParams.get("right") || stylesMeta[1]?.slug || "";
  ...
  // Get display data from pre-loaded map
  const leftData = stylesData[leftSlug];
  const rightData = stylesData[rightSlug];

  const getStyleName = (slug: string) => {
    return stylesMeta.find((s) => s.slug === slug)?.name || slug;
  };

  // Render the active tab content for one side
  const renderPanel = (slug: string, data: StyleDisplayData | undefined) => {
    if (!data) {
      ...
      case "colors":
        return (
          <div className="p-6 bg-zinc-50 dark:bg-zinc-900 rounded-lg">
            <ColorComparePanel colors={data.colors} />
          </div>
        );
      case "rules":
        return (
          <div className="p-6 bg-zinc-50 dark:bg-zinc-900 rounded-lg max-h-[500px] overflow-y-auto">
            <RulesComparePanel doList={data.doList} dontList={data.dontList} />
          </div>
        );
      case "philosophy":
        return (
          <div className="p-6 bg-zinc-50 dark:bg-zinc-900 rounded-lg max-h-[500px] overflow-y-auto">
            <PhilosophyComparePanel philosophy={data.philosophy} />
          </div>
        );
      default:
        return null;
          ...
          <div className="max-w-7xl mx-auto px-6 md:px-12">
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                {renderPanel(rightSlug, rightData)}
...
export function CompareContent({
  ...
  return (
    ...
    >
      <CompareInner stylesMeta={stylesMeta} stylesData={stylesData} />
    </Suspense>
  );
}
